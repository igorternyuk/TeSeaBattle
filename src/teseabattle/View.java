...
public class View implements ModelListener{
    ...
    private final int CELL_SIZE = FIELD_SIZE / Constants.FIELD_DIMENSION;
    ...
    ConstantModel model; //View class can call only constant methods of the model
        ...
        canvas.addMouseListener(new MouseAdapter(){
            ...
            public void mouseReleased(MouseEvent e){
                ...
                boolean isPointInsideOfTheComputerField = cursorX >= 0 &&
                        cursorX < Constants.FIELD_DIMENSION && cursorY >= 0 
                        && cursorY < Constants.FIELD_DIMENSION;
    ...
    public void draw(Graphics2D g){
        ...
        for (int i = 0; i <= 2 * Constants.FIELD_DIMENSION; ++i) {
            g.drawLine(i * CELL_SIZE, 0, i * CELL_SIZE, Constants.FIELD_DIMENSION * CELL_SIZE);
        }
        ...
        if (computerShotsNumber > 0) {
            for (int i = 0; i < computerShotsNumber; ++i) {
                g.setColor(Color.YELLOW);
                g.fillOval(Constants.FIELD_DIMENSION * CELL_SIZE + model.getComputerShotX(i)
        ...
        for (int i = 0; i < model.getHumanShipsCount(); ++i) {
            ...
            for (int j = 0; j < numCells; ++j) {
                ...
                g.fill3DRect(Constants.FIELD_DIMENSION * CELL_SIZE + x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE, CELL_SIZE, true);
            }
        }
        ...
        g.setFont(font);
        g.drawString("Human ships: " + model.getHumanNavyHealth() + "%", 40, Constants.FIELD_DIMENSION * CELL_SIZE + 40);
        g.drawString("Computer ships: " + model.getComputerNavyHealth() + "%", Constants.FIELD_DIMENSION * CELL_SIZE + 40, Constants.FIELD_DIMENSION * CELL_SIZE + 40);
        g.setColor(Color.BLUE);
        g.drawString(model.getStatus(), WINDOW_WIDTH / 2 - 100, Constants.FIELD_DIMENSION * CELL_SIZE + 70);

        //Drawing the separating line
        
       g.setColor(DARK_GREEN);
       g.setStroke(new BasicStroke(SEPARATING_LINE_WIDTH));
       g.drawLine(WINDOW_WIDTH / 2, 1, WINDOW_WIDTH / 2, Constants.FIELD_DIMENSION * CELL_SIZE - 2);
